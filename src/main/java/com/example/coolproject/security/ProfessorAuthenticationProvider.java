...
public class ProfessorAuthenticationProvider implements AuthenticationProvider {
  ...
  public Authentication authenticate(Authentication authenticationTokenToProcess) throws AuthenticationException {
    ...
    if (authenticationTokenToProcess.isAuthenticated() && authenticationTokenToProcess.getCredentials() == null &&
      ...
      // This looks like the token we manually created and authenticated after code
      // verification.
      // Avoid re-processing its (now null) credentials through
      // verifyCodeAndBuildAuthentication,
      // which would fail because the one-time code is used up from the store.
      // This situation suggests that Spring's AuthenticationManager was invoked with
      // an already-authenticated token from the SecurityContext.
      // Ideally, this provider shouldn't be called in this scenario for
      // re-authentication.
      // By returning the token as is, we prevent it from being invalidated.
      System.err.println(
          "ProfessorAuthenticationProvider.authenticate was called with an already authenticated professor token (post-verification). Returning it as is.");
    ...
    // If credentials are legitimately null for an initial authentication attempt by
    // this provider's design
    if (credentialsObj == null && !"fake@prof.com".equalsIgnoreCase(email)) {
      // For non-ping users, code (credentials) is mandatory for the initial
      // verification step handled by this provider.
      // This path might be hit if an unauthenticated token with null credentials is
      // passed.
    ...
    // (verifyCodeAndBuildAuthentication handles null code for ping and expects code
    // for others from the store)
  ...
  public Authentication verifyCodeAndBuildAuthentication(String email, String code) throws AuthenticationException {
    if (professorEmails.contains(email)) {
      ...
      boolean codeIsValid;
      if ("fake@prof.com".equalsIgnoreCase(email)) {
        if (storedCode == null) {
          codeIsValid = true;
        } else {
        ...
        if (storedCode == null) {
          throw new BadCredentialsException(
              "Verification code has expired or was not generated for this email. Please try logging in again.");
        }
        codeIsValid = storedCode.equals(code);
      }
